...
object AddressExtractor {
    ...
    /** 快递运单号行（品牌+快递后缀+冒号/空格+长数字串），如 中通快递:79152640318774——非地址。 */
    ...
    for (lineIdx in lines.indices) {
        ...
        for (p in EXPLICIT_LABELS) {
            ...
            // 而同屏另有更具体的完整街道地址行（快递正文，如 长兴路北段老李超市旁边）。
    ...
    if (st.fullAddress.isEmpty()) {
        ...
        if (labelLine != null) {
            ...
            // ②标签值退化(短/OCR读重如 地址:长兴路长兴路新)时，取「标签行下方邻近」的干净完整地址，
    ...
    // 例：LINE[取件地址 y=942 x=107] + LINE[长兴路北段店 y=942 x=380] + LINE[长兴路…老李超市 y=1032 x=380]
    if (st.fullAddress.isEmpty()) {
        ...
        for (labLine in lines) {
            ...
            // （例：取值行=长兴路北段店，续行=长兴路长兴路长兴路北段老李超市旁边 → 只用续行）
    ...
    fun extractAddressForCode(lines: List<OCREngine.TextLine>, code: String): String {
        ...
        // 真机/短信典型：凭9-4-1526到建设南路取您的快递；而相邻卡片的「凭3-7-4162到长兴路北段店」
        ...
        // 地址可能跨行（LINE8"…到长兴路与长青街…社区卫生" + LINE9"所对面2号柜H36…取您的快递"）
        ...
        // 优先级 2：地址: 标签
        // 优先级 2：地址: 标签（含退化标签补全——如 地址:长兴路长兴路新 时取下方干净地址行）
    ...
    private fun stripBrackets(s: String): String {
        ...
        // 处理不闭合的左括号（OCR/UI 折叠截断导致如 "【长兴路北段店" 无右括号）：剥掉孤立左括号
    ...
    /** 折叠连续 3 次以上重复的相邻片段（OCR 常把路名/站名读重，如 长兴路长兴路长兴路→长兴路）。
    ...
    private fun isAddressLike(s: String): Boolean {
        ...
        // Exclude 快递运单号行："品牌+快递后缀+冒号/空格+长数字串"（如 中通快递:79152640318774）
        ...
        // 「商品」单独排除会误杀真实地址「商品街」（如 长兴路商品街），仅当不含「商品街」时才排除
